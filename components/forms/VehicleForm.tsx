...
const MaintenanceEditor: React.FC<{
    ...
    onAdd: (record: Omit<MaintenanceRecord, 'id' | 'date'>) => void;
...
const VehicleForm: React.FC<VehicleFormProps> = ({ initialData, onClose }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setMaintenanceRecords(initialData.maintenance || []);
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (initialData?.id) {
      updateVehicle({ ...initialData, ...finalVehicleData });
    } else {
    ...
    const handleAddMaintenance = (record: Omit<MaintenanceRecord, 'id' | 'date'>) => {
